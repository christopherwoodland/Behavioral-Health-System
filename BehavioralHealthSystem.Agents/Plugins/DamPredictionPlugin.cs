...
public class DamPredictionPlugin
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_options.MockMode)
        {
            ...
            _logger.LogInformation(
                "[{PluginName}] DAM_MOCK_MODE enabled. Returning mock session id {SessionId}",
                nameof(DamPredictionPlugin), mockSessionId);
        ...
        catch (HttpRequestException ex) when (_options.MockMode)
        {
            ...
            _logger.LogWarning(ex,
                "[{PluginName}] DAM initiate unreachable. DAM_MOCK_MODE enabled, continuing with mock session {SessionId}",
                nameof(DamPredictionPlugin), mockSessionId);
            return mockSessionId;
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogInformation(
                "[{PluginName}] [{CorrelationId}] DAM_MOCK_MODE enabled. Returning deterministic mock prediction.",
                nameof(DamPredictionPlugin), correlationId);
            return CreateMockPrediction(sessionId);
        }
        ...
        {
            _logger.LogWarning(ex,
                "[{PluginName}] [{CorrelationId}] DAM predict unreachable. Returning mock prediction because DAM_MOCK_MODE is enabled.",
                nameof(DamPredictionPlugin), correlationId);
            return CreateMockPrediction(sessionId);
        }
    ...
    private static PredictionResponse CreateMockPrediction(string sessionId)
    {
        ...
        return new PredictionResponse
        {
            ...
            PredictedScoreAnxiety = "0.31",
            Provider = "local-dam-mock",
            CreatedAt = now.ToString("O"),
            UpdatedAt = now.ToString("O"),
            IsCalibrated = false,
            ModelCategory = "mock",
            ModelGranularity = "mock"
        };
    }
...
public class DamPredictionPluginOptions
{
    ...
    /// <summary>
    /// Enables mock DAM responses when local model server is unavailable.
    /// Set via DAM_MOCK_MODE or LOCAL_DAM_MOCK_MODE.
    /// </summary>
