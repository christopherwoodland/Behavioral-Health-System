...
public class FoundryDeepAnalysisAgentService : IDeepAnalysisAgentService
{
    private static readonly string[] TokenScopes = ["https://ai.azure.com/.default"];
    private readonly HttpClient _httpClient;
    private readonly TokenCredential _credential;
    private readonly FoundryDeepAnalysisOptions _options;
    private readonly ILogger<FoundryDeepAnalysisAgentService> _logger;

    public FoundryDeepAnalysisAgentService(
        HttpClient httpClient,
        IOptions<FoundryDeepAnalysisOptions> options,
        ILogger<FoundryDeepAnalysisAgentService> logger)
        : this(httpClient, new DefaultAzureCredential(), options, logger)
    {
    }

    internal FoundryDeepAnalysisAgentService(
        HttpClient httpClient,
    ...
    public async Task<string?> GenerateAssessmentAsync(string clinicalData, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                _logger.LogError(
                    "Foundry deep-analysis agent {AgentName} returned {StatusCode}: {ResponseBody}",
                    _options.AgentName,
        ...
        {
            _logger.LogError("Foundry deep-analysis agent {AgentName} timed out", _options.AgentName);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error invoking Foundry deep-analysis agent {AgentName}", _options.AgentName);
            return null;
        }
    }
    ...
    private string? ExtractOutputText(string responseBody)
    {
        ...
        {
            _logger.LogWarning("Foundry deep-analysis response did not contain an output array");
            return null;
        }
        ...
        _logger.LogWarning("Foundry deep-analysis response did not contain output text");
        return null;
    }
}
